...
# Placeholder for COLUMN_DEFINITIONS (to be filled in by the user)
COLUMN_DEFINITIONS = {
    's_no': {
...
def load_data():
    db_path = 'E:\\ingres_sih-main\\ingres_new\\gec.db'
    if not os.path.exists(db_path):
        raise FileNotFoundError(f"Database file not found at {db_path}")
    conn = sqlite3.connect(db_path)
    query = "SELECT * FROM gec_data"
...
# Clean None values in key columns like state and district
...
# Initialize embedding model for semantic column matching
try:
...
def extract_relevant_columns(query):
    """
    Identifies all related columns using semantic similarity and predefined relationships.
    ...
    # Get embeddings for query and column meanings
    ...
    # Select columns with high similarity (threshold can be adjusted)
    threshold = 0.3  # Adjust based on testing
    relevant_columns = [column_names[i] for i, sim in enumerate(similarities) if sim > threshold]
    
    # Include explicitly mentioned columns (direct matching as fallback)
    ...
    # Include related columns from COLUMN_DEFINITIONS
    final_columns = set(relevant_columns)
    ...
    # Always include state and district for context
    base_columns = ['state', 'district'] if 'state' in df.columns and 'district' in df.columns else []
    final_columns.update(base_columns)
    
    return list(final_columns) or list(COLUMN_DEFINITIONS.keys())[:5]  # Default to top 5 if none matched

# Create text chunks with only relevant columns
def create_text_chunk(row, relevant_columns):
    """
    Creates text chunks for relevant columns only, including meaning, categorization, and classification.
    """
    try:
        chunks = []
        for col in relevant_columns:
        ...
        print(f"Error creating text chunk for row: {e}")
        traceback.print_exc()
        return ""
...
try:
    ...
    try:
        chroma_client.delete_collection("gec_data")
    except:
        pass
...
# Ollama LLM with fallback
try:
    log_memory_usage()
    llm = OllamaLLM(model="gemma2:2b", base_url="http://localhost:11434")
except Exception as e:
    print(f"Error initializing gemma2:2b: {e}")
    traceback.print_exc()
    print("Trying fallback model phi3:mini...")
    try:
        llm = OllamaLLM(model="phi3:mini", base_url="http://localhost:11434")
    except Exception as e:
        print(f"Error initializing phi3:mini: {e}")
        traceback.print_exc()
        llm = None
...
# Retrieve function with semantic column selection
def retrieve(query, k=5):
    """
    Retrieves relevant rows and creates context with related columns.
    """
    try:
        ...
        block_match = re.search(r'\bblock\s*:\s*([a-zA-Z\s]+)\b|\b([a-zA-Z\s]+)\s*block\b', query.lower())
        ...
        # Get relevant columns using semantic similarity and relationships
        ...
        # Create context with only relevant columns
...
# Minimal static response fallback
...
# Comprehensive prompt for LLM
prompt = PromptTemplate(
    input_variables=["query", "context", "relevant_defs"],
    template="""You are an expert on groundwater data in India. Use this data context (only relevant and related columns included):
{context}

Column mappings with meanings, categorization, and classification:
{relevant_defs}

Handle the query dynamically based on its content: {query}
- Use only the provided columns and their mappings (meaning, categorization, classification).
- Include related columns (e.g., recharge for rainfall queries) as provided in the context.
- For column-specific queries: Extract the value, include its meaning, apply categorization ranges, and use classification parameters.
- For status/category queries: Use stage_of_ground_water_extraction_total (if in context) to determine category based on its classification parameters.
- For replenishment/time-related queries: Calculate net = recharge - extraction using relevant columns. Apply categorization and classification.
- For comparisons: Compare values across rows for relevant and related columns, using their mappings.
- For trends/historical: If no year data, state 'No historical data available'.
- For what-if scenarios: Hypothesize using formulas from mappings, re-categorize and re-classify.
- For educational queries: Provide meaning, categorization, and classification from mappings.
- For saline or unsupported queries: Use classification parameters or state 'No specific data available'.
- Always provide specific numerical values with units from meanings.
- If data unavailable, state 'Data not available'.
- Be concise, factual, respond in English only.
- If visualization requested, output 'VISUALIZE: [type, e.g., bar plot of rainfall by district]' at the end."""
)
...
def is_absurd(answer, query):
    ...
    if any(k in query.lower() for k in ['rainfall', 'recharge', 'extraction', 'availability', 'stage']) and not re.search(r'\d+\.?\d* (mm|ham|%)', answer):
        return True
    if 'VISUALIZE' in answer.upper() and not any(k in query.lower() for k in ['plot', 'chart', 'graph', 'visualize', 'map']):
        return True
    return False
...
def visualize(query, relevant_df):
    query_lower = query.lower()
    if not any(keyword in query_lower for keyword in ['plot', 'chart', 'graph', 'visualize', 'map', 'display']):
        return ""
    
    plt.figure(figsize=(12, 8))
    try:
        if 'map' in query_lower:
            return "Map visualization not supported in console mode. Consider using geospatial libraries like geopandas for maps."
            ...
            if all(s > 0 for s in sizes):
                ...
                plt.title(f'Extraction vs Recharge Pie Chart for {row.get("district", "Unknown")}')
            else:
...
def chat():
    ...
    while True:
        ...
        # Extract relevant and related columns
        ...
        if llm and chain:
            try:
                ...
                if is_absurd(answer, query_en):
                    ...
                    try:
                        answer = ollama.generate(model='gemma2:2b', prompt=direct_prompt)['response']
                    except Exception as e:
        ...
        viz_msg = ""
        if "VISUALIZE:" in answer.upper():
            viz_msg = visualize(query_en, relevant_df)
        elif any(k in query_en.lower() for k in ['plot', 'chart', 'graph', 'visualize', 'map', 'pie']):
            viz_msg = visualize(query_en, relevant_df)
        if viz_msg:
            answer += f"\n{viz_msg}"
