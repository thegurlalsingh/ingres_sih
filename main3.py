...
def translate_text(text, source_lang='auto'):
    try:
        ...
        # Encode and translate to English
        ...
        translated_text = translator_tokenizer.decode(generated_tokens[0], skip_special_tokens=True)
        return translated_text, source_lang
    except Exception as e:
        print(f"Error translating text: {e}")
        return text, 'en'  # Fallback to original text and assume English
...
def retrieve(query, batch_data, k=5):
    try:
        ...
        return context, [item[0] for item in relevant_rows]
    except Exception as e:
...
- If visualization is requested, output 'VISUALIZE: [type, e.g., bar plot of rainfall by district]' at the end.
- If data is unavailable (N/A), state 'Data not available'.
- Be concise, factual, and respond in English."""
)

def visualize(query, batch_data, relevant_rows):
    query_lower = query.lower()
    if not any(keyword in query_lower for keyword in ['plot', 'chart', 'graph', 'visualize', 'pie']):
        return ""
    
    try:
        if 'map' in query_lower:
            return "Map visualization not supported in console mode. Consider using a UI with geospatial libraries."
        
        relevant_columns = extract_relevant_columns(query)
        numeric_columns = [col for col in relevant_columns if col in [
            'rainfall_mm_total', 'ground_water_recharge_ham_total',
        ...
        # Extract data from relevant rows
        labels = []
        values = []
        for row in relevant_rows[:5]:  # Limit to top 5 for visualization
            district = row['metadata'].get('district', 'Unknown')
            for col in numeric_columns:
                # Extract value from text field using regex
                pattern = rf"{col.replace('_', ' ').title()}: (\S+)"
                match = re.search(pattern, row['text'])
                if match and match.group(1) != 'N/A':
                    try:
                        value = float(match.group(1))
                        labels.append(f"{district} ({col})")
                        values.append(value)
                    except ValueError:
                        continue
        
        if not values:
            return "Insufficient numeric data for visualization."
        
        # Choose chart type
        chart_type = 'pie' if 'pie' in query_lower else 'bar'
        unit = 'mm' if 'rainfall' in numeric_columns[0] else '%' if 'stage' in numeric_columns[0] else 'ham'
        
        # Generate chartjs configuration
        chart_config = {
            "type": chart_type,
            "data": {
                "labels": labels,
                "datasets": [{
                    "label": f"{numeric_columns[0].replace('_', ' ').title()} ({unit})",
                    "data": values,
                    "backgroundColor": [
                        "#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF"
                    ] if chart_type == 'pie' else "#36A2EB",
                    "borderColor": "#ffffff",
                    "borderWidth": 1
                }]
            },
            "options": {
                ...
                "plugins": {
                    ...
                    "title": {
                        "display": True,
                        "text": f"{numeric_columns[0].replace('_', ' ').title()} by District"
                    }
                }
                ...
                "plugins": {
                    ...
                    "title": {
                        "display": True,
                        "text": f"{numeric_columns[0].replace('_', ' ').title()} Distribution"
                    }
                }
            }
        }
        ...
        print("=== End of ChartJS Configuration ===")
        return f"VISUALIZE: {chart_type} plot of {numeric_columns[0]} by district"
...
def generate_response(query, batch_data):
    # Translate query to English if needed
    translated_query, source_lang = translate_text(query)
    if source_lang != 'en':
        print(f"Translated query from {source_lang} to English: {translated_query}")
    
    # Generate prompt
    relevant_columns = extract_relevant_columns(translated_query)
    relevant_defs = '\n'.join([
    ...
    context, relevant_rows = retrieve(translated_query, batch_data)
    full_prompt = prompt_template.format(query=translated_query, context=context, relevant_defs=relevant_defs)
    ...
    try:
        ...
        # Check for visualization request in query or response
        viz_msg = ""
        if any(keyword in translated_query.lower() for keyword in ['plot', 'chart', 'graph', 'visualize', 'pie']) or "VISUALIZE:" in response.upper():
            viz_msg = visualize(translated_query, batch_data, relevant_rows)
            if viz_msg and "VISUALIZE:" not in response:
                response += f"\n{viz_msg}"
...
def chat():
    ...
    if not batch_data:
        print("No batch data available. Ensure 'gec_batches.json' exists.")
        return
    
    print("Groundwater Analysis Chatbot Ready! Enter a query (type 'quit' to exit).")
    while True:
        query = input("\nYou: ").strip()
        if query.lower() == 'quit':
            break
        response = generate_response(query, batch_data)
        print("\nBot:", response)
